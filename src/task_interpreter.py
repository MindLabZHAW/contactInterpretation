...
class TaskInterpreter:
    def __init__(self, robot: RobotInterface, ai_model: ContactAI, default_contact_actions: Dict, data_logger: Optional[DataLogger] = None, loop_delay: float = 0.01):
        self.robot = robot
        self.ai = ai_model
        self.task = None
    ...
    def execute_step(self, step: Dict):
        ...
        while self.robot.is_performing_action():
            ...
            if robot_data:
                ...
                self.detection_ground_truths.append(robot_data.get("label", 0))

                if self.data_logger:
                    log_entry = {
                        'label': robot_data.get("label", 0),
                        'raw_pred': raw_pred,
